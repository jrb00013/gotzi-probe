"""
Gotzi Web UI — FastAPI backend + Wireshark-like packet browser (live capture).
Run: python -m udp_probe --web [--port 8080]
Requires root for raw socket capture. Open http://localhost:8080
"""
...
def run_web(port: int = 8080, filter_port: Optional[int] = None) -> None:
    ...
    try:
        _, _stop_event = start_live_sniffer_thread(_store, filter_port=filter_port)
